...
import (
    ...
    "ekyu.moe/util/cli"
    b64 "github.com/quycao/clitool/pkg/base64"
...
func base64Main() {
    ...
    if answers.Confirm == true {
        ...
        if answers.Mode == "Encode" {
            ...
            err = b64encode(outFilename, inFilename, wrap)
        } else {
            if len(outFilename) == 0 && inFilename != "-" {
            ...
            err = b64decode(outFilename, inFilename)
        }
...
func b64encode(outFilename, inFilename string, wrap int) error {
    ...
    if wrap <= 0 {
        e = base64.NewEncoder(base64.StdEncoding, outFile)
    } else {
        e = b64.NewLineWrapper(outFile, wrap)
    }
    defer e.Close()
...
func b64decode(outFilename, inFilename string) error {
    // validate and read in file
    inFile, _, err := cli.AccessOpenFile(inFilename)
    if err != nil {
        return err
    ...
    d := base64.NewDecoder(base64.StdEncoding, inFile)
